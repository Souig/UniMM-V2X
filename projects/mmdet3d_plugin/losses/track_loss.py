...
from projects.mmdet3d_plugin.unimmv2x.dense_heads.track_head_plugin import Instances
...
class ClipMatcher(nn.Module):
    ...
    def loss_boxes(self, outputs, gt_instances: List[Instances],
        ...
        # pred_bboxes = src_boxes.clone()
        # gt_bboxes_3d = target_boxes.clone()
        # print(f"Pred BBox shape: {pred_bboxes.shape}, dtype: {pred_bboxes.dtype}, \
        #       Pred BBox min: {pred_bboxes.min().item()}, max: {pred_bboxes.max().item()}, \
        #       Pred BBox has NaN: {torch.isnan(pred_bboxes).any()}, Inf: {torch.isinf(pred_bboxes).any()}, \
        #       GT BBox shape: {gt_bboxes_3d.shape}, dtype: {gt_bboxes_3d.dtype} \
        #       GT BBox min: {gt_bboxes_3d.min().item()}, max: {gt_bboxes_3d.max().item()}, \
        #       GT BBox has NaN: {torch.isnan(gt_bboxes_3d).any()}, Inf: {torch.isinf(gt_bboxes_3d).any()}")
