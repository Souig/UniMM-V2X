...
def custom_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    ...
    if eval_planning:
        planning_metrics = PlanningMetric(n_future=planning_steps).cuda()
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            if eval_planning:
                ...
                drivable_gt = result[0]['planning']['planning_gt']['drivable_gt']
                ...
                planning_metrics(pred_sdc_traj[:, :planning_steps, :2], sdc_planning[0][0,:, :planning_steps, :2], sdc_planning_mask[0][0,:, :planning_steps, :2], segmentation[0][:, 1:planning_steps+1], drivable_gt)

            # Eval Occ
            if eval_occ:
                occ_has_invalid_frame = data['ego_agent_data']['gt_occ_has_invalid_frame'][0]
...
def custom_multi_gpu_test_wo_label(model, data_loader, tmpdir=None, gpu_collect=False):
    """Test model with multiple gpus.
    This method tests model with multiple gpus and collects the results
    under two different modes: gpu and cpu modes. By setting 'gpu_collect=True'
    it encodes results to gpu tensors and use gpu communication for results
        ...
        with torch.no_grad():
            ...
            if isinstance(result, dict):
                ...
                if eval_planning:
                    ...
                    for key in ['planning', 'sdc_boxes_3d', 'sdc_scores_3d', 
                            'sdc_track_scores', 'sdc_track_bbox_results', 'traj_0', 'traj_scores_0', 'traj_1', 'traj_scores_1', 'traj', 'traj_scores']:
                        del result[0][key]
                    for key in ['boxes_3d', 'scores_3d', 'labels_3d', 'track_scores', 'track_ids', 'boxes_3d_det', 'scores_3d_det', 'labels_3d_det']:
                        del result[0][key]
                del result[0]['track_bbox_results']
                bbox_results.extend(result)
    ...
    ret_results = dict()
    ret_results['results'] = bbox_results

    # mmcv.dump(command_list, 'commands.pkl')
