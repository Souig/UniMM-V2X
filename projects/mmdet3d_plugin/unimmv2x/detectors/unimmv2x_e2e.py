...
from .unimmv2x_track import UniMMV2XTrack
from mmdet.models.builder import build_head
import mmcv

@DETECTORS.register_module()
class UniMMV2X(UniMMV2XTrack):
    def __init__(
        self,
        seg_head=None,
        motion_head=None,
        ...
        **kwargs,
        ...
        super(UniMMV2X, self).__init__(is_cooperation=is_cooperation,
        ...
        if seg_head:
            ...
            self.seg_head.is_ego_agent = is_ego_agent
        if occ_head:
            self.occ_head = build_head(occ_head)
            self.occ_head.is_ego_agent = is_ego_agent
        if motion_head:
        ...
        self.freeze_motionformer = freeze_motionformer
    ...
    def forward_train(self,
        ...
        if self.freeze_trackformer:
            with torch.no_grad():
                losses_track, outs_track = self.forward_track_train(img, gt_bboxes_3d, gt_labels_3d, gt_past_traj, gt_past_traj_mask, gt_inds, gt_sdc_bbox, gt_sdc_label,
                                                        l2g_t, l2g_r_mat, img_metas, timestamp, other_agent_results=other_agent_results)
        ...
        if self.freeze_trackformer:
            bev_embed = bev_embed.detach().cuda()
            bev_pos = bev_pos.detach().cuda()
        ...
        if self.with_motion_head:
            if self.freeze_motionformer:
                for param in self.motion_head.parameters():
                    param.requires_grad = False
                self.motion_head.eval()
        ...
        if not self.is_ego_agent:
            unimmv2x_outs['unimmv2x_track_instances_list'] = outs_track['unimmv2x_track_instances_list']
            unimmv2x_outs['unimmv2x_track'] = outs_track
            unimmv2x_outs['unimmv2x_lane_query_data'] = outs_seg['unimmv2x_lane_query_data']
            unimmv2x_outs['unimmv2x_motion'] = outs_motion

        return losses, unimmv2x_outs
    ...
    def forward_test(self,
        ...
        if self.with_planning_head:
            result_planning = self.planning_head.forward_test(bev_embed, outs_motion, sdc_planning, outs_occ, command, drivable_pred)
            if w_label:
                planning_gt=dict(
