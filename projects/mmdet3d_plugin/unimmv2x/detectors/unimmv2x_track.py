...
class UniMMV2XTrack(MVXTwoStageDetector):
    def __init__(
        self, 
        use_grid_mask=False,
        img_backbone=None,
        ...
        super(UniMMV2XTrack, self).__init__(
        ...
        self.freeze_trackformer = freeze_trackformer
        ...
        if self.freeze_trackformer:
            self.freeze_TrackFormer()

        self.is_ego_agent = is_ego_agent
        self.return_track_query = return_track_query

    def freeze_TrackFormer(self):
        for param in self.query_embedding.parameters():
            param.requires_grad = False
        for param in self.reference_points.parameters():
            param.requires_grad = False
        for param in self.bev_embed_linear.parameters():
            param.requires_grad = False
        for param in self.bev_pos_linear.parameters():
            param.requires_grad = False
        ...
        for param in self.pts_bbox_head.parameters():
            param.requires_grad = False
        if self.is_cooperation:
            for param in self.cross_agent_query_interaction.parameters():
                param.requires_grad = False
        if self.freeze_bn:
            self.query_embedding.eval()
            self.reference_points.eval()
            self.bev_embed_linear.eval()
            self.bev_pos_linear.eval()
            self.query_interact.eval()
    ...
    def get_history_bev(self, imgs_queue, img_metas_list):
        ...
        with torch.no_grad():
            ...
            for i in range(len_queue):
                ...
                prev_bev, _, _ = self.pts_bbox_head.get_bev_features(
    ...
    def get_bevs(self, imgs, img_metas, prev_img=None, prev_img_metas=None, prev_bev=None):
        ...
        moe_loss = torch.tensor(0.0, device=imgs.device)
        if self.freeze_bev_encoder:
            with torch.no_grad():
                bev_embed, bev_pos, _ = self.pts_bbox_head.get_bev_features(
                    mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev)
        else:
            bev_embed, bev_pos, moe_loss = self.pts_bbox_head.get_bev_features(
                    mlvl_feats=img_feats, img_metas=img_metas, prev_bev=prev_bev)
        ...
        return bev_embed, bev_pos, moe_loss
    ...
    def _get_coop_bev_embed(self, bev_embed_src, bev_pos_src, track_instances, start_idx):
        ...
        # act_track_instances = track_instances
    ...
    def _forward_single_frame_train(
        ...
        bev_embed, bev_pos, moe_loss = self.get_bevs(
            img, img_metas,
        ...
        if self.is_ego_agent and self.is_cooperation and other_agent_results:
            ...
            for other_agent_name, other_agent_result in other_agent_results.items():
                other_agent_track_instances = other_agent_result['unimmv2x_track_instances_list'][unimmv2x_frame_id]
    ...
    def forward_track_train(self,
        ...
        out = dict()
        out['unimmv2x_track_instances_list'] = []
        moe_loss = torch.tensor(0.0, device=img.device)
        ...
        for i in range(num_frame):
            ...
            moe_loss += frame_res["moe_loss"]

            # used for ego-agent fusion
            if not self.is_ego_agent and self.return_track_query:
                out['unimmv2x_track_instances_list'].append(track_instances)
