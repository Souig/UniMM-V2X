...
import numpy as np
from projects.mmdet3d_plugin.unimmv2x.fusion_modules.traj_fusion import TrajFusion
...
class MotionHead(BaseMotionHead):
    ...
    def forward_train(self,
        ...
        moe_loss = outs_motion.pop('moe_loss', None)
        loss_inputs = [gt_bboxes_3d, gt_fut_traj, gt_fut_traj_mask, outs_motion, all_matched_idxes, track_boxes]
        losses = self.loss(*loss_inputs)
        losses['moe_loss'] = torch.tensor(moe_loss, device=bev_embed.device) if moe_loss is not None else torch.tensor(0.0, device=bev_embed.device)
        ...
        if self.is_fusion and other_agent_results is not None:
            outs_motion = self.traj_fusion(other_agent_results, outs_motion, track_query)
    ...
    def forward_test(self, bev_embed, outs_track={}, outs_seg={}, other_agent_results=None):
        """Test function"""
    ...
    def forward(self, 
        ...
        inter_states, inter_references, moe_loss = self.motionformer(
