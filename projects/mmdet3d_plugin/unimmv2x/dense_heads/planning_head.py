...
class PlanningHeadSingleMode(nn.Module):
    def __init__(self,
        ...
        self.guidance_proj = nn.Linear(2, 256)
        self.alpha = alpha
    ...
    def forward_train(self,
        ...
        outs_planning = self(bev_embed, occ_mask, bev_pos, sdc_traj_query, sdc_track_query, sdc_planning, command, None)
    ...
    def forward_test(self, bev_embed, outs_motion={}, sdc_planning=None, outs_occflow={}, command=None, drivable_pred=None):
        sdc_traj_query = outs_motion['sdc_traj_query']
        sdc_track_query = outs_motion['sdc_track_query']
        bev_pos = outs_motion['bev_pos']
        occ_mask = outs_occflow['seg_out']
        
        outs_planning = self(bev_embed, occ_mask, bev_pos, sdc_traj_query, sdc_track_query, sdc_planning, command, drivable_pred)
    ...
    def forward(self, 
        ...
        Args:
            ...
            bev_pos (torch.Tensor): BEV position.use_col_optim
        ...
        if isinstance(sdc_embedding, list):
            sdc_embedding = sdc_embedding[0]
        sdc_embedding = sdc_embedding.to(plan_query.dtype)

        planning_embedding = sdc_embedding.detach()
        planning_embedding_agg = planning_embedding[0, :, :self.planning_steps, :2].mean(dim=1, keepdim=True)
        
        planning_embedding_proj = self.guidance_proj(planning_embedding_agg)  # (1, 1, 256)

        plan_query = self.alpha * plan_query + (1 - self.alpha) * planning_embedding_proj
        anchor = sdc_embedding[0, :, 0:1, :2].to(plan_query.dtype)
        sdc_traj = sdc_embedding[0, :, :self.planning_steps, :2].to(plan_query.dtype)
        ...
        # sdc_traj_all = sdc_traj + sdc_traj_all
        sdc_traj_all = anchor + torch.cumsum(sdc_traj_all, dim=1)
