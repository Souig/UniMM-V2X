...
class PlanningMetric(Metric):
    ...
    def update(self, trajs, gt_trajs, gt_trajs_mask, segmentation, drivable_gt):
        '''
        trajs: torch.Tensor (B, n_future, 3)
        gt_trajs: torch.Tensor (B, n_future, 3)
        segmentation: torch.Tensor (B, n_future, 200, 200) 1 for occupancy; 0 for free
        drivable_gt: torch.Tensor (200, 200) 1 for drivable; 0 for undrivable
        '''
        ...
        # evaluate out of boundary
        B, n_future, _ = trajs.shape
        drivable_gt = drivable_gt.unsqueeze(0).unsqueeze(0).repeat(B, n_future, 1, 1)
        undrivable_area = 1 - drivable_gt
        obj_out_sum, obj_box_out_sum = self.evaluate_coll(trajs[:,:,:2], gt_trajs[:,:,:2], undrivable_area)
