...
import mmcv, os
from ..fusion_modules import LaneQueryFusion
...
class PansegformerHead(SegDETRHead):
    ...
    def __init__(
            ...
            *args,
            ...
            return_lane_query=True,
            save_lane_query_file_root='',
            is_cooperation=True,
            is_ego_agent=False,
            read_lane_query_file_root='',
            is_bev_aug=False,
            **kwargs):
        self.bev_h = bev_h
        self.bev_w = bev_w
        ...
        self.return_lane_query = return_lane_query
        self.save_lane_query_file_root = save_lane_query_file_root
        self.is_cooperation = is_cooperation
        self.is_ego_agent = is_ego_agent
        self.read_lane_query_file_root = read_lane_query_file_root
        self.is_bev_aug = is_bev_aug
        if self.is_cooperation:
            self.cross_lane_fusion = LaneQueryFusion(pc_range=self.pc_range,
    ...
    def _get_coop_bev_embed(self, memory_src, memory_pos_src, lane_query, lane_query_pos, reference, start_idx=300):
        bev_embed = memory_src
        bev_pos = memory_pos_src
        other_lane_query = lane_query[0][start_idx:]
        other_lane_query_pos = lane_query_pos[0][start_idx:]
        other_reference = reference[0][start_idx:]

        locs = other_reference.sigmoid().clone()
        locs[:, 0:1] = locs[:, 0:1] * self.bev_w # w
        locs[:, 1:2] = locs[:, 1:2] * self.bev_h # h

        pixel_len = 2

        bev_embed_new = bev_embed.clone()
        bev_pos_new = bev_pos.clone()
        for idx in range(other_reference.shape[0]):
            w = int(locs[idx, 0])
            h = int(locs[idx, 1])
            ...
            for hh in range(max(0, h - pixel_len), min(self.bev_h - 1, h + pixel_len)):
                for ww in range(max(0, w - pixel_len), min(self.bev_w - 1, w + pixel_len)):
                    bev_embed_new[:, hh * self.bev_w + ww, :] =  bev_embed[:, hh * self.bev_w + ww, :] + self.bev_embed_linear(other_lane_query[idx])
                    bev_pos_new[:, hh * self.bev_w + ww, :] =  bev_pos[:, hh * self.bev_w + ww, :] + self.bev_pos_linear(other_lane_query_pos[idx])

        return bev_embed_new, bev_pos_new

    @force_fp32(apply_to=('bev_embed', ))
    def forward(self, bev_embed, img_metas=None, other_agent_results=None, forward_mode='train'):
        """Forward function.
        ...
        if self.is_ego_agent and self.is_cooperation and other_agent_results:
            for other_agent_name, other_agent_result in other_agent_results.items():
                if forward_mode == 'train':
                    other_agent_lane_data = other_agent_result['unimmv2x_lane_query_data']
                    ...
                    other_agent_lane_data = other_agent_result[0]['unimmv2x_lane_query_data']
    ...
    def forward_test(self,
        ...
        pred_seg_dict = self(pts_feats, img_metas=img_metas, other_agent_results=other_agent_results, forward_mode='test')

        unimmv2x_lane_save_data = None
        if not self.is_ego_agent and self.return_lane_query:
            memory, memory_mask, memory_pos, query, _, query_pos, hw_lvl = pred_seg_dict['args_tuple']
            unimmv2x_lane_query_data = {'outputs_classes': pred_seg_dict['outputs_classes'],
                        'outputs_coords': pred_seg_dict['outputs_coords'],
                        'query': query,
                        'query_pos': query_pos,
                        'reference': pred_seg_dict['reference']}
        ...
        if not self.is_ego_agent and self.return_lane_query:
            bbox_list[0]['unimmv2x_lane_query_data'] = unimmv2x_lane_query_data

        return bbox_list, drivable_gt, drivable_pred
    ...
    def forward_train(self,
        ...
        """
        pred_seg_dict = self(bev_feat, img_metas=img_metas, other_agent_results=other_agent_results, forward_mode='train')
        ...
        if not self.is_ego_agent and self.return_lane_query:
            ...
            unimmv2x_lane_query_data = {'outputs_classes': pred_seg_dict['outputs_classes'],
                        ...
                        'query_pos': query_pos,
                        'reference': pred_seg_dict['reference']}
            pred_seg_dict['unimmv2x_lane_query_data'] = unimmv2x_lane_query_data
