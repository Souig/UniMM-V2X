    ...
    def forward_test(
        ...
        if self.is_ego_agent and self.is_cooperation and other_agent_results:
            for other_agent_name, other_agent_result in other_agent_results.items():
                if 'unimmv2x_occ_prob_data' not in other_agent_results[other_agent_name][0]['occ']:
                    import pdb;pdb.set_trace()
                other_agent_occ_data = other_agent_results[other_agent_name][0]['occ']['unimmv2x_occ_prob_data']
        ...
        if not self.is_ego_agent and self.return_occ_data:
            out_dict['unimmv2x_occ_prob_data'] = pred_seg_scores[0]
