...
class AgentQueryFusion(nn.Module):
    ...
    def visualize_ref_pts_matplotlib(self, veh_pts, inf_pts_transformed, output_name):

        fig = plt.figure(figsize=(10, 8))
        ax = fig.add_subplot(111, projection='3d')
        
        veh_pts = veh_pts.cpu().detach().numpy()
        inf_pts_transformed = inf_pts_transformed.cpu().detach().numpy()

        ax.scatter(veh_pts[:, 0], veh_pts[:, 1], veh_pts[:, 2],
                c='blue', label='Vehicle Ref Points', s=10)

        ax.scatter(inf_pts_transformed[:, 0], inf_pts_transformed[:, 1], inf_pts_transformed[:, 2],
                c='red', label='Inf Ref Points (Transformed)', s=10)
    ...
    def _dis_filt(self, veh_pts, inf_pts, veh_dims):
        ...
        """
        diff = torch.abs(veh_pts - inf_pts)
    ...
    def _query_fusion(self, inf, veh, inf_idx, veh_idx, cost_matrix):
        ...
        veh_pos_embed = self.get_pos_embedding(veh.ref_pts[veh_accept_idx])
        inf_pos_embed = self.get_pos_embedding(inf.ref_pts[inf_accept_idx])
        Q = self.q_proj(veh.query[veh_accept_idx, self.embed_dims:] + veh_pos_embed)
        K = self.k_proj(inf.query[inf_accept_idx, self.embed_dims:] + inf_pos_embed)
        V = self.v_proj(inf.query[inf_accept_idx, self.embed_dims:] + inf_pos_embed)
        attn = (Q @ K.transpose(-2, -1)) * (self.embed_dims ** -0.5)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        query = veh.query[veh_accept_idx, :self.embed_dims] + self.cross_agent_fusion(attn @ V)
        veh.query[veh_accept_idx, self.embed_dims:] = query
    ...
    def forward(self, inf, veh, ego2other_rt, other_agent_pc_range, threshold=0.3):
        ...
        veh_mask_new = torch.where(veh.obj_idxes>=0)
        veh_new = veh[veh_mask_new]
        veh_new_ref_pts = self._loc_denorm(veh_new.ref_pts, self.pc_range)
        ...
        # self.visualize_ref_pts_matplotlib(veh_ref_pts, inf_ref_pts, 'results/ref_pts_before_transform.png')
        # self.visualize_ref_pts_matplotlib(veh_new_ref_pts, inf_ref_pts, 'results/ref_pts_before_transform_veh_new.png')
            
        # inf_ref_pts inf2veh
        calib_inf2veh = np.linalg.inv(ego2other_rt[0].cpu().numpy().T)  #TODO
        ...
        # matching
        veh_mask = torch.where(veh.scores > 0.03)[0]
        # self.visualize_ref_pts_matplotlib(veh_ref_pts, inf_ref_pts, 'results/ref_pts.png')
        # if len(veh_mask_new) != 0:
        #     import pdb; pdb.set_trace()
        # print(veh.scores, inf.scores)
        # print(veh_ref_pts, inf_ref_pts)
        # veh_mask = torch.ones_like(veh.scores, dtype=torch.bool)
        ...
        # i = torch.randint(0, 1000, (1,)).item()  # Randomly select an iteration number for logging
        # if i  == 250:
        #     print(f'veh_accept_id: {veh_accept_idx}, inf_accept_id: {inf_accept_idx}' )
