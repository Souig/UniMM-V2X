...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False, category_convert_type='detection_category', dataset_type='nuScenes'):
    """
    ...
    if  dataset_type == 'spd':
        import mmcv
        def create_splits_spd(split_datas_file):
        ...
        splits = create_splits_spd(split_datas_file)
    else:
...
def load_gt_v2xsim(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False, category_convert_type='detection_category', dataset_type='nuScenes'):
    """
    Loads ground truth boxes from DB.
    :param nusc: A NuScenes instance.
    :param eval_split: The evaluation split for which we load GT boxes.
    ...
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            if box_cls == DetectionMotionBox_modified:
                ...
                fut_traj_scence_centric = np.zeros((0,))
                if fut_traj_local.shape[0] > 0:
                    _, boxes, _ = nusc.get_sample_data(sample['data']['LIDAR_TOP_id_2'], selected_anntokens=[sample_annotation['token']])
                    box = boxes[0]
                    trans = box.center
                    rot = Quaternion(matrix=box.rotation_matrix)
                    fut_traj_scence_centric = convert_local_coords_to_global(fut_traj_local, trans, rot) 
                ...
                sample_boxes.append(
                    box_cls(
                        ...
                        size=sample_annotation['size'],
                        rotation=sample_annotation['rotation'],
                        velocity=nusc.box_velocity(sample_annotation['token'])[:2] / 200000,
                        num_pts=sample_annotation['num_lidar_pts'][1],
                        detection_name=detection_name,
                        detection_score=-1.0,  # GT samples do not have a score.
                        attribute_name=attribute_name,
                        visibility=sample_annotation['visibility_token'],
