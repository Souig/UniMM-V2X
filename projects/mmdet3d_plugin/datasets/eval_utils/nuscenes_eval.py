...
from nuscenes.eval.common.loaders import load_prediction, add_center_dist, filter_eval_boxes #load_gt
...
from mmdet3d.datasets import Whales
from mmdet3d.datasets import WhalesEval, WhalesTrackingEval
from mmdet3d.datasets.whales_eval import WhalesDetectionBox, WhalesTrackingBox, WhalesDetectionConfig, WhalesDetectionMetrics
...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False, splits={}, category_to_type_name=None):
    """
    ...
    # if 'nuScenes' in nusc.dataroot:
    #     splits = create_splits_scenes()
    # else:
    #     splits = create_splits_spd()
    ...
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            if box_cls == DetectionBox_modified:
                ...
                # else:
                #     detection_name = class_names_nuscenes_mappings[sample_annotation['category_name']]

                detection_name = category_to_type_name(sample_annotation['category_name'])
                ...
                # else:
                #     tracking_name = class_names_nuscenes_mappings[sample_annotation['category_name']]

                tracking_name = category_to_type_name(sample_annotation['category_name'])
...
class TrackingEval_custom(TrackingEval):
    def __init__(self,
        ...
        self.splits = splits
        self.category_to_type_name = category_to_type_name
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        self.pred_boxes = filter_eval_boxes(nusc, self.pred_boxes, self.class_range, verbose=verbose)
        if verbose:
            print('Filtering ground truth annotations')
        # self.gt_boxes = filter_eval_boxes(nusc, self.gt_boxes, self.cfg.class_range, verbose=verbose)
        self.gt_boxes = filter_eval_boxes(nusc, self.gt_boxes, self.class_range, verbose=verbose)
...
class WhalesEval_custom(WhalesEval):
    """
    Dummy class for backward-compatibility. Same as DetectionEval.
    """
    ...
    def __init__(self,
        ...
        if verbose:
            print('Initializing nuScenes detection evaluation')
        self.pred_boxes, self.meta = self.load_prediction(self.result_path, self.cfg.max_boxes_per_sample, WhalesDetectionBox,
                                                     verbose=verbose)
        self.gt_boxes = self.load_gt(self.nusc, self.eval_set, WhalesDetectionBox, verbose=verbose)

        assert set(self.pred_boxes.sample_tokens) == set(self.gt_boxes.sample_tokens), \
            "Samples in split doesn't match samples in predictions."

        # Add center distances.
        self.pred_boxes = self.add_center_dist(nusc, self.pred_boxes)
        self.gt_boxes = self.add_center_dist(nusc, self.gt_boxes)
        ...
        if verbose:
            print('Filtering predictions')
        # self.pred_boxes = filter_eval_boxes(nusc, self.pred_boxes, self.cfg.class_range, verbose=verbose)
        self.pred_boxes = self.filter_eval_boxes(nusc, self.pred_boxes, self.class_range, verbose=verbose)
        if verbose:
            print('Filtering ground truth annotations')
        # self.gt_boxes = filter_eval_boxes(nusc, self.gt_boxes, self.cfg.class_range, verbose=verbose)
        self.gt_boxes = self.filter_eval_boxes(nusc, self.gt_boxes, self.class_range, verbose=verbose)
    ...
    def update_gt(self, type_='vis', visibility='1', index=1):
        if type_ == 'vis':
            ...
            # self.gt_boxes = filter_by_sample_token(self.all_gt, valid_tokens)
            # self.pred_boxes = filter_by_sample_token(self.all_preds, valid_tokens)
            self.gt_boxes = self.all_gt
            self.pred_boxes = self.all_preds
        self.sample_tokens = self.gt_boxes.sample_tokens


    def evaluate(self) -> Tuple[WhalesDetectionMetrics, DetectionMetricDataList]:
        """
        Performs the actual evaluation.
        :return: A tuple of high-level and the raw metric data.
        """
        ...
        if self.verbose:
            print('Calculating metrics...')
        metrics = WhalesDetectionMetrics(self.cfg)
        for class_name in self.cfg.class_names:
            # Compute APs.
            for dist_th in self.cfg.dist_ths:
                metric_data = metric_data_list[(class_name, dist_th)]
    ...
    def render(self, metrics: WhalesDetectionMetrics, md_list: DetectionMetricDataList) -> None:
        """
        Renders various PR and TP curves.
        :param metrics: DetectionMetrics instance.
        :param md_list: DetectionMetricDataList instance.
...
class TrackingEvalWhales_custom(WhalesTrackingEval):
    def __init__(self,
                 config: TrackingConfig,
                 result_path: str,
                 eval_set: str,
        ...
        # Initialize NuScenes object.
        # We do not store it in self to let garbage collection take care of it and save memory.
        nusc = Whales(verbose=verbose, dataroot=nusc_dataroot)

        # Load data.
        if verbose:
            print('Initializing nuScenes tracking evaluation')
        pred_boxes, self.meta = self.load_prediction(self.result_path, self.cfg.max_boxes_per_sample, WhalesTrackingBox,
                                                verbose=verbose)
        gt_boxes = self.load_gt(nusc, self.eval_set, WhalesTrackingBox, verbose=verbose)

        assert set(pred_boxes.sample_tokens) == set(gt_boxes.sample_tokens), \
            "Samples in split don't match samples in predicted tracks."

        # Add center distances.
        pred_boxes = self.add_center_dist(nusc, pred_boxes)
        gt_boxes = self.add_center_dist(nusc, gt_boxes)

        # Filter boxes (distance, points per box, etc.).
        if verbose:
            print('Filtering tracks')
        # pred_boxes = filter_eval_boxes(nusc, pred_boxes, self.cfg.class_range, verbose=verbose)
        pred_boxes = self.filter_eval_boxes(nusc, pred_boxes, self.class_range, verbose=verbose)
        if verbose:
            print('Filtering ground truth tracks')
        # gt_boxes = filter_eval_boxes(nusc, gt_boxes, self.cfg.class_range, verbose=verbose)
        gt_boxes = self.filter_eval_boxes(nusc, gt_boxes, self.class_range, verbose=verbose)

        self.sample_tokens = gt_boxes.sample_tokens

        # Convert boxes to tracks format.
        self.tracks_gt = self.create_tracks(gt_boxes, nusc, self.eval_set, gt=True)
        self.tracks_pred = self.create_tracks(pred_boxes, nusc, self.eval_set, gt=False)
