...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples(self, location, ego2global_translation, ego2global_rotation, new_range_100=False):
        map_pose = ego2global_translation[:2]
        rotation = Quaternion(ego2global_rotation)

        # Mat = [cos(theta), -sin(theta); sin(theta), cos(theta)]
        # Mat * [L, 0] = [cos(theta) * L, sin(theta) * L]
        if new_range_100:
            map_pose_new = map_pose.copy()
            move_len = 51.2
            theta_r = quaternion_yaw(rotation)
            map_pose_new[0] = map_pose[0] + np.cos(theta_r) * move_len
            map_pose_new[1] = map_pose[1] + np.sin(theta_r) * move_len
            patch_box = (map_pose_new[0], map_pose_new[1], self.patch_size[0], self.patch_size[1])
        else:
