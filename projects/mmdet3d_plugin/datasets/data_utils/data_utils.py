...
def lidar_b2d_box_to_global(info,
                             boxes,
                             classes,
                             class_range,
                             eval_version='detection_cvpr_2019'):
    ...
    for i, box in enumerate(boxes):
        ...
        if radius > det_range:
            continue
        # Move box to global coord system
        box.rotate(Quaternion(matrix=np.linalg.inv(np.array(info['world2ego'])[:3, :3])))
...
def obtain_map_info(nusc,
    ...
    if new_range_100:
        ...
        e2g_t_new = e2g_t.copy()
        e2g_t_new[0] = e2g_t[0] + np.cos(theta_r) * move_len
        e2g_t_new[1] = e2g_t[1] + np.sin(theta_r) * move_len
        l2g_r_mat = (l2e_r_mat.T @ e2g_r_mat.T).T
        l2g_t = l2e_t @ e2g_r_mat.T + e2g_t_new
