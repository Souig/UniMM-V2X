...
class Visualizer:
    ...
    def __init__(
        ...
        if self.show_hd_map:
            self.nusc_maps = {
                    'yizhuang06': NuScenesMap(dataroot=dataroot, map_name='yizhuang06'),
                    'yizhuang08': NuScenesMap(dataroot=dataroot, map_name='yizhuang08'),
                    'yizhuang09': NuScenesMap(dataroot=dataroot, map_name='yizhuang09'),
                    'yizhuang10': NuScenesMap(dataroot=dataroot, map_name='yizhuang10'),
                    'yizhuang13': NuScenesMap(dataroot=dataroot, map_name='yizhuang13'),
                    'yizhuang16': NuScenesMap(dataroot=dataroot, map_name='yizhuang16')
            }
...
def main(args):
    ...
    split_data = mmcv.load(split_datas_file)
    val_splits = split_data['batch_split']['val']
    ...
    for i in range(len(viser.nusc.sample)):
        ...
        if sample_token not in viser.token_set:
            ...
            continue
        filename = str(i).zfill(4) + '_' + scene_token + '_' + sample_token
        viser.visualize_bev(sample_token, os.path.join(args.out_folder, filename))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_folder', default='./demo_test/', help='Output folder path')
    ...
    parser.add_argument('--dataroot', default='./datasets/V2X-Seq-SPD-New/vehicle-side', help='path to data')
