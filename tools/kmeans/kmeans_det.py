...
fp = 'data/infos/V2X-Seq-SPD-Sparse/infrastructure-side/spd_sparse_infos_temporal_train.pkl'
data = mmcv.load(fp)
...
plt.savefig(f'vis/kmeans/det_anchor_{K}_inf', bbox_inches='tight')
others = np.array([1,1,1,1,0,0,0,0])[np.newaxis].repeat(K, axis=0)
cluster = np.concatenate([cluster, others], axis=1)
np.save(f'data/kmeans/kmeans_det_{K}_inf.npy', cluster)