...
fp = 'data/infos/V2X-Seq-SPD-New/infrastructure-side/spd_sparse_infos_temporal_train.pkl'
data = mmcv.load(fp)
...
plt.savefig(f'vis/kmeans/plan_{K}_inf', bbox_inches='tight')
plt.close()

clusters = np.stack(clusters, axis=0)
np.save(f'data/kmeans/kmeans_plan_{K}_inf.npy', clusters)